...
$con=mysqli_connect("localhost","root","","myhmsdb");
  ...
  if (isset($_SESSION['pid'])) {
    ...
    $pid = null; // or assign a default value if needed
  }
  
  if (isset($_SESSION['username'])) {
    $username = $_SESSION['username'];
  } else {
    $username = null; // or assign a default value if needed
  }
  
  if (isset($_SESSION['email'])) {
    $email = $_SESSION['email'];
  } else {
    $email = null; // or assign a default value if needed
  }
  
  if (isset($_SESSION['fname'])) {
    $fname = $_SESSION['fname'];
  } else {
    $fname = null; // or assign a default value if needed
  }
  
  if (isset($_SESSION['gender'])) {
    $gender = $_SESSION['gender'];
  } else {
    $gender = null; // or assign a default value if needed
  }
  
  if (isset($_SESSION['lname'])) {
    $lname = $_SESSION['lname'];
  } else {
    $lname = null; // or assign a default value if needed
  }
  
  if (isset($_SESSION['contact'])) {
    $contact = $_SESSION['contact'];
  } else {
    $contact = null; // or assign a default value if needed
  }  
    ...
    if((date("Y-m-d",$appdate1)==$cur_date and date("H:i:s",$apptime1)>$cur_time) or date("Y-m-d",$appdate1)>$cur_date) {
      $check_query = mysqli_query($con,"select apptime from appointmenttb where doctor='$doctor' and appdate='$appdate' and apptime='$apptime'");
        ...
        if(mysqli_num_rows($check_query)==0){
          $query=mysqli_query($con,"insert into appointmenttb(pid,fname,lname,gender,email,contact,doctor,docFees,appdate,apptime,userStatus,doctorStatus, description) values($pid,'$fname','$lname','$gender','$email','$contact','$doctor','$docFees','$appdate','$apptime','1','1', '$description')");
...
<html lang="en">
  ...
  <a class="navbar-brand" href="index.php"><i class="fa fa-user-plus" aria-hidden="true"></i> Global Hospital </a>
